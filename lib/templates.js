  ...
  return {
    ...
    discount:
      compareAtPrice && compareAtPrice > price
        ...
        : null,
    // NOTE: notionUrl is deliberately NOT surfaced here. Everything this
    // function returns is serialised into the public page HTML, and notionUrl
    // is the private duplication link buyers pay for. Server-only code reads it
    // straight from the frontmatter instead — see getDeliveryForPriceId().
...
/**
 * SERVER ONLY. Returns raw frontmatter, including `notionUrl` — the private
 * duplication link a buyer pays for.
 *
 * Never call this from a component, and never put its result into page props.
 * Anything the browser is allowed to see goes through getAllTemplates().
 */
export function getTemplatesWithPrivateFields() {
  return getTemplateSlugs().map((slug) => ({ slug, ...readTemplateFile(slug).data }));
}
